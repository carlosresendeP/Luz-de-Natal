...
const App: React.FC = () => {
  ...
  const getNewMessage = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      do {
        ...
        nextMessage = MESSAGES[randomIndex];
  ...
  const copyToClipboard = () => {
    navigator.clipboard.writeText(currentMessage.text).then(() => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-between text-white relative overflow-hidden font-sans pb-10">
      ...
      <main className="relative z-20 flex-1 flex flex-col items-center justify-center w-full max-w-4xl px-6">
        ...
        <div className={`w-full glass p-8 md:p-12 rounded-[2rem] text-center transition-all duration-500 transform ${isAnimating ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
          <div className="mb-6 flex justify-center">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 text-[#D2122E] animate-float" fill="currentColor" viewBox="0 0 24 24">
              <path d="M12 2L4.5 20.29L5.21 21L12 18L18.79 21L19.5 20.29L12 2Z" fill="#D4AF37" />
              <path d="M12 12L12 15" stroke="white" strokeWidth="2" strokeLinecap="round" />
            </svg>
          </div>
      ...
      <footer className="relative z-20 text-center px-4 w-full">
        ...
        <div className="flex items-center justify-center gap-2 text-white/40 text-xs">
          ...
          <span>•</span>
          <a href="#" className="hover:text-[#D4AF37] transition-colors underline decoration-dotted">Compartilhe Amor</a>
        </div>
      </footer>
